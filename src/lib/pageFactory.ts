...
export async function loadAllDatasets(): Promise<PageDatasets> {
  ...
  for (const p of priorities) {
    ...
    priorityData[p.slug] = omitMissingYears(rows, p.excludeYears)
  }

  const stratifiedData: Record<string, StratifiedRow[]> = {}
  for (const ind of indicators) {
    const rows = await tryLoad<StratifiedRow>(
      ind.slug, ind.file, ind.scheme, { territory: app.local },
    )
...
export function buildPages(datasets: PageDatasets): PageDefinition[] {
  ...
  const priorityPages: PageDefinition[] = priorities.flatMap((priority: IndicatorMeta) => [
    {
      slug: `analisis-de-inequidad/${priority.slug}`, title: priority.title,
      text: priority.description, date: priority.date, category: priority.category,
      source: priority.source, navbar: false, data: datasets.priorityData[priority.slug],
    },
    {
      slug: `determinantes-de-la-salud/${priority.slug}`, title: priority.title,
      text: priority.description, date: priority.date, source: priority.source,
      category: priority.category, navbar: false,
    },
    {
      slug: `analisis/${priority.slug}`, title: `Análisis de ${priority.label}`,
      description: 'Indicadores de análisis de datos y visualización.', date: priority.date,
      category: 'Tendencia', navbar: false, priority: false,
      forestPlotData: datasets.forestPlotData, analyticsData: datasets.analyticsData,
      scatterData: datasets.scatterData,
    },
  ])

  const indicatorPages: PageDefinition[] = indicators.map((ind) => ({
    slug: ind.slug, title: ind.title, description: ind.description,
    dimension: ind.dimension, subdimensions: ind.subdimensions, date: ind.date,
    navbar: false, stratifiers: ind.stratifiers, source: ind.source,
...
export async function buildStaticPaths() {
  ...
  return pages.map(({ slug, title, description, date, navbar, source, ...rest }) => ({
    params: { slug },
    props: {
      title, description, slug, date: new Date(date), navbar, source, pages, ...rest,
    },
  }))
}
